...
func (fb *Filebase) Reserve(id types.FileID) (types.FileID, error) {
	...
	for out == nil {
		...
		if err != nil {
			return id, srverror.New(err, 500, "Error F1", "Unable to update id reserve")
		}
...
func (fb *Filebase) Insert(r types.FileI) error {
	...
	if err != nil {
		return srverror.New(err, 500, "Error F2", "Unable to insert")
	}
	if result.ModifiedCount == 0 {
...
func (fb *Filebase) Get(fid types.FileID) (types.FileI, error) {
	...
	if err := result.Decode(fd); err != nil {
		...
		return nil, srverror.New(err, 500, "Error F3", "Unable to get file")
	}
	f := fd.File()
...
func (fb *Filebase) Update(r types.FileI) error {
	...
	if err != nil {
		return srverror.New(err, 500, "Error F4", "error updating file")
	}
	if result.ModifiedCount == 0 {
...
func (fb *Filebase) Remove(r types.FileID) error {
	...
	if err != nil {
		return srverror.New(err, 500, "Error F5", "unable to remove file", r.String())
	}
...
func (fb *Filebase) decodefiles(cursor *mongo.Cursor) ([]types.FileI, error) {
	...
	if err := cursor.All(fb.ctx, &reference); err != nil {
		...
		return nil, srverror.New(err, 500, "Error F6", "unable to decode file list")
	}
...
func (fb *Filebase) GetOwned(uid types.OwnerID) ([]types.FileI, error) {
	...
	if err != nil {
		...
		return nil, srverror.New(err, 500, "Error F7", "unable to send request")
	}
	return fb.decodefiles(cursor)
}
...
func (fb *Filebase) GetPermKey(uid types.OwnerID, pkey string) ([]types.FileI, error) {
	...
	if err != nil {
		...
		return nil, srverror.New(err, 500, "Error F8", "unable to send request")
	}
	return fb.decodefiles(cursor)
}
...
func (fb *Filebase) Count(uid types.OwnerID, pkeys ...string) (int64, error) {
	...
	if err != nil {
		...
		return -1, srverror.New(err, 500, "Error F10", "unable to count documents")
	}
	return count, nil
}
...
func (fb *Filebase) MatchStore(oid types.OwnerID, sid []types.StoreID, pkeys ...string) ([]types.FileI, error) {
	...
	if err != nil {
		...
		return nil, srverror.New(err, 500, "Error F9", "unable to send request")
	}
	return fb.decodefiles(cursor)
}
